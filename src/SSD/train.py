...
def train_model(mode, config_path, model_dir, num_steps):
    ...
    with mlflow.start_run(run_name=run_name):
        pipline_config = config_util.get_configs_from_pipeline_file(config_path)
        ...
        if mode == "initial":
            ...
            print("Fine-tuning with adjusted hyperparameters")
        elif mode == "optimized":
            with open(config_path, "r") as f:
                optimized_params = yaml.safe_load(f)
            train_config["optimizer"]["momentum_optimizer"]["learning_rate"]["cosine_restart_learning_rate"]["initial_learning_rate"] = optimized_params["learning_rate"]
            train_config["batch_size"] = optimized_params["batch_size"]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_path", default = "src/SSD/ssd_pipeline.config", type=str, help="path to config file")
